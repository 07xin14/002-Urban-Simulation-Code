...
if __name__ == '__main__':
    ...
    ### Scenarios B1: change the beta from 100% to 150% ###
    beta2 = beta*1.5
    
    cdata["prodsimest4"] = np.exp(cdata["alpha_i"]+gamma*np.log(cdata["jobs"]) - beta2*cdata["log_distance"])

    cdata["prodsimest4"] = round(cdata["prodsimest4"],0)
    ...
    cdatamat4.to_csv(path_out+'cdatamat4.csv',index=True)
    
    #cdatamat2.insert(0,"station_origin",cdatamat2.index)
    #.index = cdatamat.index
    store_difference(cdatamat,cdatamat4,'4')
    with open(path_out+'prodsim4.txt','w') as f:
        f.write(' r^2 : '+str(CalcRSqaured(cdata["flows"],cdata["prodsimest4"])))
        f.write('\n RMSE : '+str(CalcRMSE(cdata["flows"],cdata["prodsimest4"])))
        
    #calculate some new wj^alpha and d_ij^beta values
    Dj3_gamma = cdata["jobs"]**gamma
    dist_beta = cdata["distance"]**(-1.0*beta2)
    ...
    cdata["prodsimest4_4"] = cdata["A_i3"]*cdata["O_i"]*Dj3_gamma*dist_beta
    #round
    cdata["prodsimest4_4"] = round(cdata["prodsimest4_4"])
    #check
    
    cdatamat4_4 = cdata.pivot_table(values ="prodsimest4_4", index="station_origin", columns = "station_destination",
                            aggfunc=np.sum, margins=True)
    cdatamat4_4.to_csv(path_out+'cdatamat4_4.csv',index=True)
    
    #cdatamat2.insert(0,"station_origin",cdatamat2.index)
    #.index = cdatamat.index
    with open(path_out+'prodsim4_4.txt','w') as f:
        f.write(' r^2 : '+str(CalcRSqaured(cdata["flows"],cdata["prodsimest4_4"])))
        f.write('\n RMSE : '+str(CalcRMSE(cdata["flows"],cdata["prodsimest4_4"])))
    store_difference(cdatamat,cdatamat4_4,'prodsimest4_4')
        
    ### Scenarios B2: change the beta from 100% to 150% ###
    beta3 = beta*2.0
    
    cdata["prodsimest5"] = np.exp(cdata["alpha_i"]+gamma*np.log(cdata["jobs"]) - beta3*cdata["log_distance"])

    cdata["prodsimest5"] = round(cdata["prodsimest5"],0)
    #now we can convert the pivot table into a matrix
    cdatamat5 = cdata.pivot_table(values ="prodsimest5", index="station_origin", columns = "station_destination",
                            aggfunc=np.sum, margins=True)
    cdatamat5.to_csv(path_out+'cdatamat5.csv',index=True)
    
    #cdatamat2.insert(0,"station_origin",cdatamat2.index)
    #.index = cdatamat.index
    store_difference(cdatamat,cdatamat5,'5')
    with open(path_out+'prodsim5.txt','w') as f:
        f.write(' r^2 : '+str(CalcRSqaured(cdata["flows"],cdata["prodsimest5"])))
        f.write('\n RMSE : '+str(CalcRMSE(cdata["flows"],cdata["prodsimest5"])))
    
    #calculate some new wj^alpha and d_ij^beta values
    Dj3_gamma = cdata["jobs"]**gamma
    dist_beta = cdata["distance"]**(-1.0*beta3)
    #calcualte the first stage of the Ai values
    cdata["Ai1"] = Dj3_gamma * dist_beta
    #now do the sum over all js bit
    A_i = pd.DataFrame(cdata.groupby(["station_origin"])["Ai1"].agg(np.sum))
    ...
    #to check everything works, recreate the original estimates
    cdata["prodsimest5_5"] = cdata["A_i4"]*cdata["O_i"]*Dj3_gamma*dist_beta
    #round
    cdata["prodsimest5_5"] = round(cdata["prodsimest5_5"])
    #check
    
    cdatamat5_5 = cdata.pivot_table(values ="prodsimest5_5", index="station_origin", columns = "station_destination",
                            aggfunc=np.sum, margins=True)
    cdatamat5_5.to_csv(path_out+'cdatamat5_5.csv',index=True)
    
    #cdatamat2.insert(0,"station_origin",cdatamat2.index)
    #.index = cdatamat.index
    with open(path_out+'prodsim5_5.txt','w') as f:
        f.write(' r^2 : '+str(CalcRSqaured(cdata["flows"],cdata["prodsimest5_5"])))
        f.write('\n RMSE : '+str(CalcRMSE(cdata["flows"],cdata["prodsimest5_5"])))
    store_difference(cdatamat,cdatamat5_5,'prodsimest5_5')
    
    
    cdata.to_csv(path_out+'cdata_new5.csv',index=False)
    ### plot the png we need ###
    plots(path_out,path_out+"cdata_new5.csv",'prodsimest4_4')
    plots(path_out,path_out+"cdata_new5.csv",'prodsimest5_5')
    plot_difference(path_out,path_out+'prodsimest4_4_difference.csv','prodsimest4_4')
    plot_difference(path_out,path_out+'prodsimest5_5_difference.csv','prodsimest5_5')